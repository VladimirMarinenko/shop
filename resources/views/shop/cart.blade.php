                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <form action="{{ route('cart.remove', $product->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-secondary rounded-circle" style="width:32px;height:32px;padding:0;">
                                                <i class="bi bi-dash"></i>
                                            </button>
                                        </form>
                                        <span class="fw-bold mx-1 text-center" style="min-width:30px;">{{ $cart[$product->id] }}</span>
                                        <form action="{{ route('cart.add', $product->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-primary rounded-circle" style="width:32px;height:32px;padding:0;">
                            ...
                            <td class="text-center pe-3">
                                <div class="d-flex justify-content-center gap-2 flex-wrap">
                                    <a href="{{ route('cart.checkout') }}" class="btn btn-success btn-sm rounded-pill px-4">
                                        <i class="bi bi-credit-card me-1"></i> Оформить
                                    </a>
